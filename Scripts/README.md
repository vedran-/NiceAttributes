...
- **Collection foldout state is now per-collection** — expanding one list no longer collapses all others
- **Settings persist across domain reloads** — Enable/Disable Nice Attributes toggle is now saved

### Code Quality
- **Codebase refactored for maintainability** — two god classes (ClassContext: 689→71 lines, NiceEditorGUI: 510→48 lines) decomposed into focused, testable components
- **Duplicate code eliminated** — shared rendering pipeline and conditional evaluation extracted to single source of truth
- **Dead code removed** — all `#if false` blocks, commented-out code, and debug artifacts cleaned up
- **Redundant math parser removed** — consolidated to single MathematicalParser
- **Automated tests added** — NUnit test suite with comprehensive MathematicalParser tests
